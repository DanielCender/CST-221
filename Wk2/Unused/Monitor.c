...
#define SEM_MUTEX_WRITE "/sem-mutex-write"
#define SEM_MUTEX_READ "/sem-mutex-read"
...
int monitor_Initialized()
{
    ...
    if (((monitor_data.OKtoWrite) = sem_open(SEM_MUTEX_WRITE, O_CREAT, 0660, 1)) != SEM_FAILED && ((monitor_data.OKtoRead) = sem_open(SEM_MUTEX_READ, O_CREAT, 0660, 1)) != SEM_FAILED) {
        returnValue = 0;
    } else {
...
{
    sem_unlink(SEM_MUTEX_READ);
    sem_unlink(SEM_MUTEX_WRITE);
}
